...
contract Lending is CCIPReceiver, OwnerIsCreator {
    ...
    constructor(
        ...
        dataFeedETHToUSD = AggregatorV3Interface(_priceFeed);
    ...
    function calculateReward(address userAddr) public view returns (uint256) {
        ...
        int currentScale = getChainlinkDataFeedForETHUSD();

        uint256 reward = (ethInGwei * intervals * rewardRate) /
            uint(currentScale);
    ...
    function getAPR() public view returns (uint256) {
        ...
        uint256 apr = (rewardRate * intervalsPerYear * 10000) /
            uint(currentScale);
        return apr;
    }
    ...
    function redeemaToken() external {
        ...
        require(
            address(this).balance >= totalETHToReturn,
        ...
        )
    {
        ...
        if (remainingEth > 0) {
            (bool success, ) = originalSender.call{value: remainingEth}("");
    ...
    function _calculateLoanTokenAmountFromETH(
        uint256 ethAmount
    ) public view returns (uint256) {
        (, int256 ethUsdPrice, , , ) = dataFeedETHToUSD.latestRoundData();
    ...
    function checkCollateralHealth(
        address user
    )
        external
        view
